...
import { getAssets, getExchange } from "../../services/coinApi";
...
const Home: FC = (): JSX.Element => {
  ...
  const [coinData, setCoinData] = useState<ICoinData>({ price: 0.0, date: "" });
  ...
  useEffect(() => {
    ...
    const getData = async (): Promise<void> => {
      if (currentValue) {
        setIsLoad(false);
        const chartData = await getExchange(currentValue);
    ...
    <div>
      {isLoadList ? (
        <Select
          ...
          options={selectData as ISelectValue[]}
          selectParam={"asset_id"}
          name={"coin"}
        />
      ) : (
        <Loader />
